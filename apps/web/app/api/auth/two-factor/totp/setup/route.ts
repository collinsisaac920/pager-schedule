...
import { hashBackupCodes, serializeBackupCodeStorage } from "@calcom/features/auth/lib/backupCodeVerifier";
...
async function postHandler(req: NextRequest) {
  ...
  // Generate 10 backup codes — 5 random bytes each, hex-encoded (10 chars, 40 bits entropy).
  // Only SHA-256 hashes are stored; plaintext is returned to the user once and never persisted.
  const backupCodes = Array.from(Array(10), () => crypto.randomBytes(5).toString("hex"));
  const storage = { version: 2 as const, codes: hashBackupCodes(backupCodes) };
  ...
  await prisma.user.update({
    ...
    data: {
      backupCodes: symmetricEncrypt(serializeBackupCodeStorage(storage), encKey),
      twoFactorEnabled: false,
      twoFactorSecret: symmetricEncrypt(secret, encKey),
    },
  });
